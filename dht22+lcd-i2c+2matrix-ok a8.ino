...
// 2018/05/28 
// compiled with core 0.5.4
// blynk 0.4.7
// blynk to 0.5.1
// blynk local to 0.36.5
...
// Byklin Auth key
char auth1[] ="5b752cbeca284df19a1c0d8deb1f1b88";
// local blynk lugia
char auth2[] ="c3ab4ab0345d4d81934600ed4ac1dadc";

char localServer[]="lugia.synology.me";
//char localServer[]="192.168.31.249";
...
#define CHECK_DHT_PERIOD    30   //check DHT every 10 min
...
unsigned long blynkServerTimer;
...
void setup() {
    ...
    //blynkLocal = new Blynk.Blynk(auth2, options= {addr:"192.168.31.249"});
...
//lcd->print(WiFi.RSSI());
    ...
    //Blynk.begin(auth,"lugia.synology.me");
    bBlynkServer=FALSE;
 //   Blynk.begin(auth1);
    if (bBlynkServer){
        Blynk.begin(auth2,localServer,8080);
    
        blynkServerTimer = millis();
...
void loop() {
    ...
    // if ((millis()-blynkServerTimer > 120*1000L) || (millis()<blynkServerTimer)) {
    //     blynkServerTimer = millis();
    //     bBlynkServer = !bBlynkServer;
    //     // change server
    //     if (bBlynkServer) {
    //         terminal.println("switch to BLYNK");
    //         Blynk.disconnect();
    //         Blynk.begin(auth1);
    //         Blynk.connect();
            
    //     } else {
    //         //Blynk.begin(auth2,localServer );
    //         // change to IP address, to prevent timeout
    //         Blynk.disconnect();
    //         Blynk.begin(auth2, localServer);
    //         Blynk.connect();
    //         terminal.println("switch to lugia");
    //         if (!Blynk.connected()) {
    //             Blynk.begin(auth1);
    //             Blynk.connect();
    //             bBlynkServer =!bBlynkServer;
    //             terminal.println("switch fail.switch to BLYNK");
    //         }
    //     }
    ...
    if ((millis()-btTimer>30*1000L) || (millis()<btTimer)){
        ...
        if (Serial1.available()) {
            ...
            if (readByte == '#') {
                ...
                if (bBlynkServer){terminal.print(checkSum2);}
            ...
            if (bBlynkServer){terminal.flush();}
        }
    ...
    // internal DHT22
    if ((millis()-lastUp>(CHECK_DHT_PERIOD*1000L)) || (millis()<lastUp)){  
        lcdBacklightOn();
        lastUp=millis();
        ledStatus(led, 1,300);
